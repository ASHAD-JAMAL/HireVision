...
function Footer() {
  return (
    <div className="max-w-screen-2xl mx-auto px-4 md:px-20 lg:px-40 bg-white text-[#02015a] dark:bg-black dark:text-white">
      <footer className="flex flex-col md:flex-row justify-between items-start py-16 px-6 sm:px-12 lg:px-20 space-y-8 md:space-y-0">
        <aside className="flex-shrink-0 py-5 md:py-0">
          <svg
        ...
        <nav className="flex flex-col md:flex-row md:space-x-12 lg:space-x-20">
          {[
            ["Features", "Pricing", "Use Case", "Community"],
            ["About us", "Contact", "Jobs", "Press kit"],
          ].map((links, idx) => (
            <div key={idx} className="flex flex-col space-y-2 md:space-y-4">
              {links.map((link) => (
                <a
                  key={link}
                  className="text-inherit hover:text-[#02015a] dark:text-white"
                >
                  {link}
                </a>
              ))}
